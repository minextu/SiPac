...
class SiPac_Layout
{
  ...
  private function generate_js()
  {
	...
	foreach ($this->chat->channel->arr as $key => $channel)
    {
		if ($key != 0)
			$js_chat = $js_chat.",";
 ...
 public  function load()
  {
    $is_mobile = check_mobile();
     ...
     $layout_path = dirname(__FILE__) . "/../../../themes/".$this->chat->settings->get('theme')."_mobile";
     
    if ($is_mobile && is_dir($layout_path))
      $this->is_mobile = true;
    else
      $layout_path = dirname(__FILE__) . "/../../../themes/".$this->chat->settings->get('theme');
