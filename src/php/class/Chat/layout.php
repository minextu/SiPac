...
trait Chat_layout
{
  ...
  private function generate_smileys()
  {
	...
	foreach ($this->settings['smileys'] as $smiley_code => $smiley_url)
	{
		...
		$chat_smileys = $chat_smileys . "<span style='margin-right: 3px;' onclick='chat_objects[chat_objects_id[\"".$this->id."\"]].add_smiley(\" " . $smiley_code . "\");'><img$width$height src='" . $smiley_url . "' title='" . $smiley_code . "' alt='" . $smiley_code . "'></span>";
	}
	return $chat_smileys;
  }
