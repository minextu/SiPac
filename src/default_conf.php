...
function command_help($command_syntax=false)
	{	
		...
		if ($command_syntax == false)
			$array = array('info_type' => "info", 'info_text' => "<||t4||>:"); //You can use the following Commands
		else
			$array = array('info_type' => "info", 'info_text' => "<||t5||>:"); //Command Syntax
...
function command_me($text, $user=0)
	{
		...
		if (!empty($user) AND $chat_settings['can_write_as_fake_user'] OR $user == 0)
			...
			return array('info_type'=>"error", 'info_text'=>"<||t8||>"); //no permissions
	}
function command_afk($status)
	{
		...
		if ($chat_settings['deactivate_afk'] == false)
		{
			...
			return array('info_type'=>"error", 'info_text'=>"AFK is not enabled!");
	}
function command_name($new_name, $user=0)
	{
		...
		if (!empty($new_name) AND $chat_settings['can_rename'])
				...
				if (!empty($user) AND $chat_settings['can_rename_others'])
					{
						...
						if (mysql_num_rows($count) > 0)
							...
							return array('info_type'=>"error", 'info_text'=>"<||t9|'".addslashes($user)."'||>"); //User %1 not found
					}
				else if (empty($user))
					{
			...
			return array('info_type'=>"warn", 'info_text'=>"<||t10||>"); //You didn't enter a new name!
		else
			return array('info_type'=>"error", 'info_text'=>"<||t8||>"); //no permissions
	}	
function command_join($channel=0)
{
	...
	if ($chat_settings['can_join_channels'] == true OR array_search($channel, $chat_settings['channels']) !== false)
	{
		if (!empty($channel))
		{
			...
			return array('info_type'=>"error", 'info_text'=>"You must enter a channel!"); 
	}
	else
		return array('info_type'=>"error", 'info_text'=>"<||t8||>"); //no permissions
}
function command_systemmessage($message)
{
	global $chat_active_channel;
		if (empty($message))
			{
				return array('info_type'=> "info", 'info_text'=> "Keine Systemnachricht angegeben!"); //show a message
			}
		else
...
function command_answer($user, $message)
{
  ...
  {
    return array("info_type" => "error", "info_text" => "Kein Benutzer oder keine Nachricht angegeben!");
  }else if($user == $_SESSION[$chat_id]['chat_username'])
  {
    return array("info_type" => "error", "info_text" => "Sie k&ouml;nnen sich nicht selber antworten!");
  }else
  {
	...
	if(!empty($user))
 	{
		if ($chat_settings['can_kick'])
		{
			...
			if (mysql_num_rows($count) > 0)
			{
				...
				return array('info_type'=>"info", 'info_text'=>"<||t21|'".addslashes($user)."'||>"); //User %1 will be kicked
			}
			else
				return array('info_type'=>"error", 'info_text'=>"<||t9|'".addslashes($user)."'||>"); //User %1 not found
		}
		...
		return array("info_type" => "error", "info_text" => "Kein Benutzer angegeben!");
}
	...
	if(!empty($user))
 	{
		...
		return array("info_type" =>"error", "info_text" => "Ungültige Zeit Angabe!");
	}
	else
		return array("info_type" => "error", "info_text" => "Kein Benutzer angegeben!");
}
...
function command_debug($type)
{
	...
	if ($type == "warn" OR $type == "all")
			...
			if ($chat_settings['can_see_debug'])
				{
					...
					return array("info_type" => "success", "info_text" => "Debug aktiviert!");
				}
			else
				return array('info_type'=>"error", 'info_text'=>"<||t8||>"); //no permissions
		}
	else if ($type == "off")
		{
			...
			return array("info_type" => "success", "info_text" => "Debug deaktiviert!");
		}
	else
		return array("info_type" => "warn", "info_text" => "type muss warn, all oder off sein (siehe /help debug)");
}
