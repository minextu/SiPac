...
Chat.prototype.handle_chat_tasks = function(answer)
{
	...
	if (answer['get']['username'] != undefined && this.username != answer['get']['username'] && this.first_start != true || this.first_start == true)
	{
		...
		try{this.chat.getElementsByClassName('chat_username')[0].innerHTML = answer['get']['username'] + ":";}
		catch(e){}
	}
  ...
  if (answer['get']['afk'] != undefined)
	...
	try{this.chat_layout_tasks()}catch(e){};
...
Chat.prototype.handle_userlist = function(userlist_arr, channel)
{
	...
	if (userlist_arr['change_user'] != undefined)
	{
		...
		{
			document.getElementById(this.id + "_" + channel + "_user_" + userlist_arr['change_user_id'][i]).innerHTML = userlist_arr['change_user'][i];
		}
	...
	if (userlist_arr['user_writing'] != undefined)
	{
		for (var i = 0; i < userlist_arr['user_writing'].length; i++)
		{
			if (userlist_arr['user_writing'][i] == "0")
				document.getElementById(this.id + "_" + channel + "_user_" + i).getElementsByClassName("chat_speech_bubble")[0].style.width = "0px";
			else
				document.getElementById(this.id + "_" + channel + "_user_" + i).getElementsByClassName("chat_speech_bubble")[0].style.width = this.speech_bubble_width;
		}
	}
};
...
Chat.prototype.information = function(info, type, nohide, onlyhide, noclose)
{
		...
		if (onlyhide)
		...
		else
			{
				...
				var chat_num = this.num;
				if (nohide != true)
					this.info_hide_timeout = window.setTimeout(function(){ chat_objects[chat_num].information(undefined, undefined, undefined, true)}, 5000);
			}
};
...
function scroll (id, classname,speed, first)
{
	...
	if (y < id.getElementsByClassName(classname)[0].scrollHeight - id.getElementsByClassName(classname)[0].offsetHeight) 
	{
	  ...
	  else
	  {
			...
			window.setTimeout(function() { scroll(id, classname, speed) }, speed);
	  }
	}
}
